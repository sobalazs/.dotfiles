...
myScratchpads = [
  ...
  where 
    command = (myTerminal ++ " -t ncspot -e ncspot")
    find    = (title =? "ncspot")
...
--
myLayoutHook = spacingRaw True (Border 0 0 0 0) True (Border 10 10 10 10) True $ tiledThree ||| Full
  where
...
--
main = xmonad =<< statusBar myBar myPP toggleStrutKey defaults  
